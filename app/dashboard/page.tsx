...
const Dashboard = () => {
  ...
  const [todoList, setTodoList] = useState<ToDoTypes[]>([
    {
      id: 1,
      title: "Doing Workout",
      desc: "I have to do push up at 6 PM",
      date: "2024-04-30",
    },
    {
      id: 2,
      title: "Waking up",
      desc: "I have to do wake up at 6 AM",
      date: "2024-04-30",
    },
    {
      id: 3,
      title: "Doing Workout",
      desc: "I have to do pull up at 7 PM",
      date: "2024-04-30",
    },
  ]);
  const [defaultValue, setDefaultValue] = useState<ToDoTypes>({
    id: 0,
    title: "",
    desc: "",
    date: "",
  ...
  const markAsUnRead = (id: number) => {
    let result = completedTasks;
    let filter = result?.filter((item) => item !== id);
  ...
  return (
    <div className=" bg-zinc-900 w-[100%] min-h-[100vh] flex flex-col items-center pt-[40px] sm:p-[40px] gap-[20px]">
      ...
      <TodoModal
        ...
        index={index}
      />
      <DeleteModal open={deleteModal} toggle={toggle} deleteTask={deleteTask} id={defaultValue?.id}/>
      <div className="w-[100%] sm:w-[80%] lg:w-[60%] bg-zinc-800 p-[20px] flex flex-col gap-[30px]">
        <div className="flex sm:flex-row flex-col w-[100%] justify-between pb-[30px] border-b-[gray] border-b gap-[20px]">
          ...
          <button
            ...
            className="px-[16px] py-[8px] text-[20px] bg-green-500 text-white whitespace-nowrap"
          >
            Add new task
          </button>
        </div>
          ...
          {todoList?.map((item, index) => {
            return (
              <div
                key={index}
                className={
                  completedTasks.includes(item?.id) ? "hidden" : "block"
                }
              >
                <TodoCard
                  item={item}
          ...
          {todoList?.map((item, index) => {
            return (
              <div
                key={index}
                className={
                  completedTasks.includes(item?.id) ? "block" : "hidden"
                }
              >
                <TodoCard
                  item={item}
